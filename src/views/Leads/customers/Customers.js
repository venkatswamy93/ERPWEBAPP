...
import ReuseTable from "../../../components/usableComponets/ReuseTable";
import { getCustomerData, addCustomer, updateCustomerQuotation } from "./../../../components/data/customerData";
import {
  CButton,
  CCol,
  CForm,
    ...
    addedRooms.forEach((room, index) => {
      ...
      if (roomItems && roomItems.length > 0) {
        ...
        doc.autoTable({
          ...
          theme: "grid",
          didDrawCell: (data) => {
            if (data.row.index === roomItems.length) {
              const roomTotal = roomItems.reduce((sum, item) => sum + ((item.length * item.height) * item.price), 0);
